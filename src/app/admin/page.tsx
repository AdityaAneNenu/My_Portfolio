...
export default function AdminPage() {
  ...
  const saveAllData = async (updatedData: any) => {
    setIsSaving(true);
    try {
      const res = await fetch('/api/admin/portfolio-data', {
        method: 'POST',
  ...
  const handleSaveExperience = (e: React.FormEvent) => {
    ...
    if (isAddingNew) {
      updatedExp.push(editingExperience);
    } else {
      updatedExp = updatedExp.map((exp, idx) => idx === editingExperience._index ? editingExperience : exp);
    }
  ...
  const handleSaveAchievement = (e: React.FormEvent) => {
    ...
    if (isAddingNew) {
      updatedAch.push(editingAchievement);
    } else {
      updatedAch = updatedAch.map((ach, idx) => idx === editingAchievement._index ? editingAchievement : ach);
    }
        ...
        {isLoadingData || !portfolioData ? (
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-4">
              {/* Sidebar Tabs */}
              <div className="lg:col-span-3 flex flex-row lg:flex-col border-b lg:border-b-0 lg:border-r border-border pb-4 lg:pb-0 lg:pr-8 gap-1 overflow-x-auto lg:overflow-x-visible">
                {[
                  ...
                  <button
                    ...
                    className={`relative px-4 py-3 text-[10px] tracking-widest uppercase transition-colors duration-300 flex items-center gap-3 overflow-hidden cursor-pointer rounded-none min-w-[140px] lg:min-w-0 ${
                      activeTab === tab.id
                        ? 'text-bg font-bold'
